...
We use the standard KBC evaluation metrics, Precision, Recall and F1-score, to compare the predicted object-entities with true object-entities on the hidden test dataset. We release a <a href="/models/bert.py">baseline</a> implementation and <a href="/evaluation.py">evaluation</a> script. The baseline model probes the BERT langauge model using a sample prompt like _"Germany shares border with \[MASK\]"_ and selects object-entities predicted in the \[MASK\] position with $>= 0.5$ likelihood as outputs on validation subject-entities. Participants can use evaluation script to compute the precision, recall and F1-score for assessing the performance of their systems.
